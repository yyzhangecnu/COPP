...
B <- 50 #B=1 for Sweight or Dweight, B>1 for BootSweight or BootDweight
CovProb <- list()
AveLeng <- list()
ALOrac <- list()
Nt <- list()

Teta <- c(-0.5,-0.5,-0.5,-0.5,-0.5,rep(0,d-4))
Deta <- c(-0.5,1,1,-1,-1,rep(0,d-4))
...
save.image("SS_high_BootDweight.Rdata")
