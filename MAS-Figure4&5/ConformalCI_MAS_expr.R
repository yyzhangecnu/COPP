...
ConformalCI_MAS_expr <- function(Data_train,Data_val,Data_test,method,B=1,trainprop,psfun,psparams,outfun,outparams,quantiles,type,side,weighted,alpha,wthigh,wtlow){
  ...
  for (b in 1:B) {
    ...
    D <- unlist(lapply(Data_train$X, MulActPolicy,epsilon=0.2))
    mean <- Data_train$X*D
    YD <- rnorm(n*trainprop,mean,1)
