...
Sampling <- function(X, T, Y, Xtest, Ytest, Teta, Deta, DenModel,a, Pol,quantiles, type, side, alpha, wthigh, wtlow){
  ...
  #generate true monte carlo quantile of Y^{D=T}|X
  #compute the nonconformity score function
  tp_DT <- tp_D*tp_T/(tp_D*tp_T+(1-tp_D)*(1-tp_T))
  for (i in 1:n) {
    Drep <- rbinom(n = 10000, size = 1, prob=tp_DT[i])
  ...
  tp_DT_test <- tp_D_test*tp_T_test/(tp_D_test*tp_T_test+(1-tp_D_test)*(1-tp_T_test))
  for (i in 1:ntest) {
    Dtestrep <- rbinom(n = 10000, size = 1, prob=tp_DT_test[i])
