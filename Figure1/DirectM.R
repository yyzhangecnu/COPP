#DenModel is true or false
#QuanModel is true or false
#Pol is deterministic or random
#a is optional when DenModel is FALSE
Direct <- function(X, T, Y, Xtest, Ytest, Teta, Deta, QuanModel, DenModel, a, Pol,quantiles, type, side, alpha, wthigh, wtlow){
  
  if(Pol == "deterministic"){
    tp_T <- I(cbind(1,X)%*%Teta>0)
    tp_D <- I(cbind(1,X)%*%Deta>0)
    tp_T_test <- I(cbind(1,Xtest)%*%Teta>0)
    tp_D_test <- I(cbind(1,Xtest)%*%Deta>0)
  }else if(Pol == "random"){
    tp_T <- exp(cbind(1,X)%*%Teta)/(1+exp(cbind(1,X)%*%Teta))
    tp_D <- exp(cbind(1,X)%*%Deta)/(1+exp(cbind(1,X)%*%Deta))
    tp_T_test <- exp(cbind(1,Xtest)%*%Teta)/(1+exp(cbind(1,Xtest)%*%Teta))
    tp_D_test <- exp(cbind(1,Xtest)%*%Deta)/(1+exp(cbind(1,Xtest)%*%Deta))
  }
   
  n <- nrow(X)
  ntest <- nrow(Xtest)
  #Yhat for quantile estimation at lower and upper quantiles
  if(QuanModel == "TRUE"){
    Yhat <- matrix(0,n,2)
    Yhat_test <- matrix(0,ntest,2)
  }else if(QuanModel == "FALSE"){
    Yhat <- matrix(runif(2*n,0,a),n,2)
    Yhat_test <- matrix(runif(2*ntest,0,a),ntest,2)
  }
  
  #generate true monte carlo quantile of Y^{T}|X
  #compute the nonconformity score function
  for (i in 1:n) {
    Trep <- rbinom(n = 10000, size = 1, prob=tp_T[i])
    M1 <- 1+X[i,1]-X[i,2]+X[i,3]^{3}+exp(X[i,4])
    M2 <- (3-5*X[i,1]+2*X[i,2]-3*X[i,3]+X[i,4])
    M3 <- (1+X[i,1]+X[i,2]+X[i,3]+X[i,4])
    Err <- rnorm(10000)
    Yrep <- M1+Trep*M2+(1+Trep)*M3*Err
    Yhat[i,] <- Yhat[i,]+quantile(Yrep,probs = quantiles)
  }
  Yscore <- conformalScore(Y, Yhat, type, side)
  
  #generate true monte carlo quantile of Y^{T}|X for test data
  for (i in 1:ntest) {
    Ttestrep <- rbinom(n = 10000, size = 1, prob=tp_T_test[i])
    M1 <- 1+Xtest[i,1]-Xtest[i,2]+Xtest[i,3]^{3}+exp(Xtest[i,4])
    M2 <- (3-5*Xtest[i,1]+2*Xtest[i,2]-3*Xtest[i,3]+Xtest[i,4])
    M3 <- (1+Xtest[i,1]+Xtest[i,2]+Xtest[i,3]+Xtest[i,4])
    Err <- rnorm(10000)
    Ytestrep <- M1+Ttestrep*M2+(1+Ttestrep)*M3*Err
    Yhat_test[i,] <- Yhat_test[i,]+quantile(Ytestrep,probs = quantiles)
  }
  
  #weight functions
  #conditional density for Y(1)|X and Y(0)|X
  #a is a parameter to control the density shift
  n <- nrow(X)
  ntest <- nrow(Xtest)
  if(DenModel == "TRUE"){ 
    DenY1 <- rep(0,n)
    DenY0 <- rep(0,n)
    DenY1_test <- rep(0,ntest)
    DenY0_test <- rep(0,ntest)
  }else if(DenModel == "FALSE"){
    DenY1 <- runif(n,0,a)
    DenY0 <- runif(n,0,a)
    DenY1_test <- runif(ntest,0,a)
    DenY0_test <- runif(ntest,0,a)
  }
  
 #conditional density Y(1)|X Y(0)|X for data
  for(i in 1:n){
    M1 <- 1+X[i,1]-X[i,2]+X[i,3]^{3}+exp(X[i,4])
    M2 <- (3-5*X[i,1]+2*X[i,2]-3*X[i,3]+X[i,4])
    Mean1 <- M1+M2
    Mean0 <- M1
    M3 <- (1+X[i,1]+X[i,2]+X[i,3]+X[i,4])
    Sd1 <- 2*M3
    Sd0 <- M3
    DenY1[i] <- DenY1[i]+dnorm(Y[i],Mean1,Sd1)
    DenY0[i] <- DenY0[i]+dnorm(Y[i],Mean0,Sd0)
  }
  
  #conditional density for Y(1)|X and Y(0)|X for test data
  for(i in 1:ntest){
    M1 <- 1+Xtest[i,1]-Xtest[i,2]+Xtest[i,3]^{3}+exp(Xtest[i,4])
    M2 <- (3-5*Xtest[i,1]+2*Xtest[i,2]-3*Xtest[i,3]+Xtest[i,4])
    Mean1 <- M1+M2
    Mean0 <- M1
    M3 <- (1+Xtest[i,1]+Xtest[i,2]+Xtest[i,3]+Xtest[i,4])
    Sd1 <- 2*M3
    Sd0 <- M3
    DenY1_test[i] <- DenY1_test[i]+dnorm(Ytest[i],Mean1,Sd1)
    DenY0_test[i] <- DenY0_test[i]+dnorm(Ytest[i],Mean0,Sd0)
  }
  
  wt <- (tp_D*DenY1+(1-tp_D)*DenY0)/(tp_T*DenY1+(1-tp_T)*DenY0)
  wt_test <- (tp_D_test*DenY1_test+(1-tp_D_test)*DenY0_test)/(tp_T_test*DenY1_test+(1-tp_T_test)*DenY0_test)
  
  PredInt<- predict.conformalSplit(Yhat_test,Yscore,wt,wt_test,alpha,wthigh,wtlow,type,side)
  
  ind <- (Ytest<(PredInt[,2]))&(Ytest>(PredInt[,1]))
  #sum(ind)/ntest
  CovProb <- sum(ind)/ntest
  AveLeng <- mean(PredInt[,2]-PredInt[,1])
  return(list(CovProb = CovProb,AveLeng=AveLeng))
  
}